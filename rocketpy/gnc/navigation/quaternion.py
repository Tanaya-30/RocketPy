...
class Quaternion:
    ...
    def from_rotation_matrix(cls, R: NDArray[np.float64]) -> "Quaternion":
        ...
        if not np.allclose(rotation.T @ rotation, identity, atol=ORTHOGONALITY_TOL, rtol=0.0):
    ...
    def dot(self, other: object) -> np.float64:
        ...
        if not isinstance(other, Quaternion):
            raise TypeError("Quaternion dot product is only supported for Quaternion operands.")
    ...
    def to_euler(self) -> tuple[float, float, float]:
        ...
        if np.isclose(abs(r20), 1.0, atol=ANGLE_TOL):
            roll = 0.0
            yaw = np.arctan2(-rotation[0, 1], rotation[1, 1]) if r20 < 0.0 else np.arctan2(rotation[0, 1], -rotation[1, 1])
        else:
    ...
    def integrate(
        ...
        q_dot = 0.5 * Quaternion(q_current[0], q_current[1], q_current[2], q_current[3]) * omega_quaternion
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"Quaternion(w={self.w:.6f}, x={self.x:.6f}, y={self.y:.6f}, z={self.z:.6f})"
        )
    ...
    def __add__(self, other: object) -> "Quaternion":
        ...
        if not isinstance(other, Quaternion):
            raise TypeError("Quaternion addition is only supported for Quaternion operands.")
        return Quaternion(self.w + other.w, self.x + other.x, self.y + other.y, self.z + other.z)

    def __sub__(self, other: object) -> "Quaternion":
        """Subtract two quaternions component-wise."""
        if not isinstance(other, Quaternion):
            raise TypeError("Quaternion subtraction is only supported for Quaternion operands.")
        return Quaternion(self.w - other.w, self.x - other.x, self.y - other.y, self.z - other.z)
    ...
    def __mul__(self, other: object) -> "Quaternion":
        ...
        raise TypeError("Quaternion multiplication is only supported for Quaternion and scalar operands.")
